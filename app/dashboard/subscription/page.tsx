...
export default function SubscriptionPage() {
  ...
  const loadData = async () => {
    ...
    return {
      ...
      pendingRequest: requestData as SubscriptionRequest | null,
    }
  }
  ...
  const activateFreeTrial = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'تعذر تفعيل الخطة المجانية')
      }

      toast.success(result.message || 'تم تفعيل الشهر المجاني')
      await loadData()
      router.refresh()
    } catch (error: unknown) {
  ...
  const freeFeatures = ['شهر مجاني كامل', 'قالب واحد', 'منتجات غير محدودة', 'رابط فريد للمتجر']
  const standardFeatures = ['3 قوالب متجر', 'منتجات غير محدودة', 'دفع تلقائي شهري', 'يمكن استخدام كود خصم داخل Stripe Checkout']
  const proFeatures = ['كل القوالب', 'تحليلات ومظهر احترافي', 'دفع تلقائي شهري', 'يدعم أكواد الخصم داخل صفحة الدفع']
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            الاشتراكات المدفوعة أصبحت تعمل عبر Stripe مع تجديد تلقائي شهري، ودعم أكواد الخصم داخل صفحة الدفع.
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                    {subscription.current_period_end && (
                      <span className="flex items-center gap-1">
                        ...
                        التجديد / الانتهاء: {new Date(subscription.current_period_end).toLocaleDateString('ar-SA')}
                      </span>
                    )}
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-muted-foreground">/{subscription.billing_interval === 'month' ? 'شهر' : subscription.billing_interval || 'شهر'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {pendingRequest && (
        <Card className="border-warning/30 bg-warning/5">
          <CardContent className="p-5 text-sm text-muted-foreground">
            يوجد عندك طلب قديم من نظام واتساب لخطة {pendingRequest.plan_type === 'pro' ? 'Pro' : 'Standard'}.
            تقدر تتجاهله وتستخدم Stripe مباشرة من البطاقات بالأسفل.
          </CardContent>
        </Card>
      )}
      ...
      <Card className="border-dashed border-primary/40 bg-muted/20">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            أكواد الخصم والدفع التلقائي
          </CardTitle>
          <CardDescription>
            عند الضغط على الاشتراك سيتم فتح Stripe Checkout، وهناك يقدر العميل يدخل Promotion Code / Coupon ويكمل الدفع مع تجديد تلقائي شهري.
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className={`border-2 ${subscription?.plan_type === 'free' && subscription.status === 'active' ? 'border-primary' : 'border-border'}`}>
          <CardHeader className="text-center pb-8">
            ...
            <CardDescription>تجربة أولية لمدة شهر</CardDescription>
            <div className="mt-4">
          ...
          <CardContent className="space-y-6">
            ...
            {subscription?.plan_type === 'free' && subscription.status === 'active' ? (
              ...
              <Button
                ...
                disabled={isActivatingFree || Boolean(hasPaidSubscription) || actionLoading !== null}
              >
                {isActivatingFree ? <Loader2 className="h-4 w-4 animate-spin ml-2" /> : <Gift className="h-4 w-4 ml-2" />}
                تفعيل شهر مجاني
              </Button>
            )}
          </CardContent>
        ...
        <Card className={`border-2 relative overflow-hidden ${subscription?.plan_type === 'pro' && subscription.status === 'active' ? 'border-primary' : 'border-primary/50'}`}>
          ...
          <CardHeader className="text-center pb-8 pt-12">
            <Badge variant="secondary" className="mx-auto mb-4">
              يدعم أكواد الخصم داخل صفحة الدفع
            </Badge>
            <div className="w-12 h-12 mx-auto rounded-full bg-primary/10 flex items-center justify-center mb-4">
              <Crown className="h-6 w-6 text-primary" />
            </div>
